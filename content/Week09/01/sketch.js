...
// Face detection callback loop
function gotResults(err, result){
  if (err){
    console.error(err);
    setTimeout(() => faceapi.detect(gotResults), 120);
    return;
  }
  detections = result || [];
  faceapi.detect(gotResults);
...
function draw(){
  ...
  const ex = (L.x + R.x) * 0.5, ey = (L.y + R.y) * 0.5;
  const yEyeLocal = h * EYE_Y_FRAC;
  const yAxis = { x: -Math.sin(angle), y: Math.cos(angle) };
  const cx = ex - yAxis.x * yEyeLocal;
...
function lowestOnJaw(d){
  ...
  let maxY = -1e9, cx = 0;
  for (const p of d.parts.jawOutline){
    const x = p._x ?? p.x, y = p._y ?? p.y;
    if (y > maxY){
      maxY = y;
      cx = x;
    }
...
function drawFaceShape(type, w, h) {
  ...
  switch (type) {
    case 0:
      ...
      break;
    case 1:
      ellipse(0, 0, w * 0.95, h * 0.95);
      break;
    case 2:
      rectMode(CENTER);
      rect(0, 0, w * 0.90, h * 0.95, w * 0.12);
      break;
    case 3:
      push();
      translate(0, -h * 0.05);
...
function drawEars(type, w, h) {
  ...
  switch (type) {
    case 0:
      ellipse(-earCX, 0, w * 0.18, h * 0.30);
      ellipse(earCX, 0, w * 0.18, h * 0.30);
      break;
    case 1:
      ellipse(-earCX, -h * 0.02, w * 0.14, w * 0.14);
      ellipse(earCX, -h * 0.02, w * 0.14, w * 0.14);
      break;
    case 2:
      ellipse(-earCX, 0, w * 0.12, h * 0.36);
      ellipse(earCX, 0, w * 0.12, h * 0.36);
      break;
    case 3:
      rectMode(CENTER);
      rect(-earCX, 0, w * 0.16, h * 0.26, w * 0.04);
      rect(earCX, 0, w * 0.16, h * 0.26, w * 0.04);
      break;
    case 4:
      ellipse(-earCX, h * 0.08, w * 0.22, h * 0.22);
      ellipse(earCX, h * 0.08, w * 0.22, h * 0.22);
      break;
  }
  stroke(210);
  strokeWeight(2);
  noFill();
  const dW = (type === 3) ? w * 0.10 : w * 0.12;
  const dH = (type === 3) ? h * 0.18 : h * 0.20;
...
function drawEyes(type, w, h) {
  ...
  switch (type) {
    ...
    case 2:
      fill(255);
      arc(-dx, y, 42, 26, PI, 0, CHORD);
      arc(dx, y, 42, 26, PI, 0, CHORD);
      fill(0);
      ellipse(-dx, y + 2, 10, 10);
      ellipse(dx, y + 2, 10, 10);
      break;
    case 3:
      fill(255);
    ...
    case 4:
      ...
      ellipse(dx, y, 44, 24);
      fill(0);
      ellipse(-dx, y + 1, 10, 10);
      ellipse(dx, y + 1, 10, 10);
      stroke(0);
      strokeWeight(2);
      line(-dx - 22, y - 8, -dx + 22, y - 8);
      line(dx - 22, y - 8, dx + 22, y - 8);
      noStroke();
      break;
  }
...
function drawNose(type, w, h) {
  ...
  switch (type) {
    ...
    case 1:
      beginShape();
      vertex(0, 0);
      quadraticVertex(-w * 0.03, h * 0.08, 0, h * 0.14);
      endShape();
      break;
    case 2:
      line(0, 0, 0, h * 0.10);
      arc(0, h * 0.12, w * 0.10, h * 0.05, 0, PI);
      point(-w * 0.02, h * 0.12);
      point(w * 0.02, h * 0.12);
      break;
    case 3:
      ...
      vertex(0, -h * 0.02);
      vertex(0, h * 0.08);
      vertex(w * 0.03, h * 0.12);
      endShape();
      break;
    case 4:
      beginShape();
      vertex(0, -h * 0.02);
      quadraticVertex(w * 0.02, h * 0.05, -w * 0.01, h * 0.10);
      quadraticVertex(-w * 0.02, h * 0.12, 0, h * 0.14);
      endShape();
      break;
  }
}
